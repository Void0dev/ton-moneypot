...
import { MoneypotMaster } from '../build/Moneypot/tact_Moneypot';
...
(async () => {
  ...
  let publicKey = beginCell()
    .storeBuffer(tickets.publicKey)
    .endCell()
    .beginParse()
    .loadUintBig(256); // Convert to uint256

  let init = await PayoutsMaster.init(owner, publicKey);
